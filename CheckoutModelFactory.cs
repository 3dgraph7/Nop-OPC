    ...
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        public virtual async Task<OnePageCheckoutModel> PrepareOnePageCheckoutModelAsync(IList<ShoppingCartItem> cart)
        {
            ...
            if (model.ShippingRequired)
                model.ShippingAddress = await PrepareShippingAddressModelAsync(cart, prePopulateNewAddressWithCustomerFields: true);

            ////discount code
            //model.DiscountBox.Display = true;

            ////order review
            //model.OrderReview = PrepareOrderReviewModel(cart);

            //shipping methods
            var shippingMethodModel = await PrepareShippingMethodModelAsync(cart, await _customerService.GetCustomerShippingAddressAsync(await _workContext.GetCurrentCustomerAsync()));
            ...
            model.IsPaymentWorkflowRequired = await _orderProcessingService.IsPaymentWorkflowRequiredAsync(cart, false);
            if (model.IsPaymentWorkflowRequired)
            {
                ...
                model.PaymentMethod = paymentMethodModel;
            }
            else
            {
            ...
            model.ShoppingCart = await _shoppingCartModelFactory.PrepareShoppingCartModelAsync(model.ShoppingCart, cart, prepareAndDisplayOrderReviewData: true);

            //confirm order
            model.ConfirmOrder = await PrepareConfirmOrderModelAsync(cart);
